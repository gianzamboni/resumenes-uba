...
\begin{center}
	...
	\node[] (glabel) [below=of v3, xshift=0.95cm] {Puentes: \((v_1,v_8), (v_2, v_4), (v_3, v_5)\)};
	
	\path
	(v1) edge[red, line width=0.75mm] (v8)
	(v2) edge (v3)
	(v2) edge[red, line width=0.75mm]  (v4)
	(v2) edge (v6)
	(v3) edge[red, line width=0.75mm]  (v5)
	(v3) edge (v6)
	(v4) edge (v7)
	(v4) edge (v8)
	(v7) edge (v8)
...
\begin{theorem}
	\begin{itemize}
		\item Un árbol m-ario de altura \(h\) tiene a lo sumo \(m^h\) hojas.
		\item Un árbol m-ario con \(l\) hojas tiene \(h \geq \lceil \log_m{l}\rceil\)
...
\begin{demo}
\begin{itemize}
	\item Hacemos inducción en la altura del árbol. Caso base \(h = 1\)
	\begin{center}
		\begin{tikzpicture}[graphStyle]
			\node[basicNode, scale=0.37] (v1) at (0,0) {};
			\node[basicNode, scale=0.37] (v2) [below=of v1] {};
			\node[basicNode, scale=0.37] (v3) [left=of v2] {};	
			\node[basicNode, scale=0.37] (v4) [right=of v2] {};
			\node[] (v1br) [below right=of v1] {};
			\node[] (v1bl) [below left =of v1] {};
			\path
			(v1) edge (v2)
			(v1) edge (v3)
			(v1) edge (v4)
