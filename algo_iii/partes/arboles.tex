...
Para implementar un \textbf{BFS} un hay que utilizar una cola (\texttt{Queue}), para un \textbf{DFS}, una cola (\textbf{Stack}).
		      ...
		      \begin{itemize}
			      ...
			      \item Si \(\blue{e}\notin X_T\), veamos que podemos armar un AGM a partir de \(T\) que contenga \red{\(T_{k-1}\)} y a \(\blue{e}\):
			            ...
			            \begin{itemize}
				            ...
				            \item \(T'\) es AGM de G: Como \orange{\(f\)} era una arista elegible al comienzo de la iteración \(k\), pero el algoritmo eligió \blue{e}, seguro se cumple \(l(\blue{e}) \leq l(\blue{f})\).
			            \end{itemize}
		      \end{itemize}
	\end{itemize}
\end{demo}
	...
	\begin{itemize}[label={}]
		\item
		      \begin{itemize}[label={}]
			      \item
			            \begin{itemize}[label={}]
				            \item  Entonces \[l(T') = l(\violet{T}) + l(\blue{e}) - l(\blue{f}) \leq \violet{T}\]
