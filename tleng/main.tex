\input{../document.setup}

\title{Teoría de Lenguajes}
\author{Gianfranco Zambonni}
\input{../page.setup}
\input{custom.comands.tex}
\theoremstyle{definition}
\newtheorem{definition}{Definición}[section]
\newtheorem{teorema}{Teorema}[section]
\newtheorem{lemma}{Lema}[section]

\begin{document}
\maketitle
\maketitle
\tableofcontents
\newpage

\part{La que da Julio}
\input{secciones/julio/introduccion.tex}
\newpage
\input{secciones/julio/automatas.tex}

\newpage
\input{secciones/julio/expresiones.regulares.tex}

\newpage
\input{secciones/julio/minimizacion.afd.tex}

\newpage
\input{secciones/julio/lenguajes.regulares.tex}

\newpage
\input{secciones/julio/automatas.pila.tex}

\newpage
\input{secciones/julio/gramaticas.independientes.tex}

\newpage
\input{secciones/julio/maquinas.turing.tex}

\newpage
\part{La que da Vero}
\include{sections/vero/gramaticas.chomsky.tex}
\end{document}